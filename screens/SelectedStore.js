...
const SelectedStore = ({ navigation, route }) => {

  
   
    // const { StoreCode } = route.params;
    ...
    React.useEffect(() => {
        const unsubscribe = navigation.addListener('focus', async () => {
            ...
            setLoading(true)
            const storeCodeSelected = route.params.StoreCode;
            console.log('New ' + storeCodeSelected)

            fetch(URL+'index.php/store_management/loadstorebasicdata?store_code=' + storeCodeSelected)
            .then((response) => response.json())
            .then((json) => {
                console.log(json.store_info[0]);
                setStoreCode(json.store_info[0].store_code)
                ...
                // console.log(json.store_info);
                return json.store_category;
            })
            .catch((error) => {
                console.error(error);
            });
